...
function NavLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link to={to} className="text-sm text-[#8BA4B8] hover:text-white transition-colors">
      {children}
    </Link>
  )
}
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#071521]/80 backdrop-blur-md border-b border-[rgba(0,119,182,0.1)]">
      <div className="mx-auto max-w-7xl flex items-center justify-between px-4 h-14">
        <Link to="/" className="flex items-center gap-2 font-bold text-lg text-[#0077B6]">
        ...
        <nav className="flex items-center gap-3">
          {user ? (
            <>
              ...
              <Link to="/profile" className="flex items-center gap-2 text-sm text-white hover:text-[#0077B6] transition-colors">
