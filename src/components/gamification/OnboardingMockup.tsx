...
const STEPS = [
  {
    ...
    gradient: 'from-[#0077B6]/30 to-transparent',
  },
  {
    icon: '📍',
    ...
    gradient: 'from-[#FFD100]/30 to-transparent',
  },
  {
    icon: '🎟️',
...
export function OnboardingMockup() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-6">
      <div className={`w-full max-w-sm rounded-2xl p-8 bg-[#0D2137] border border-[rgba(0,119,182,0.1)] bg-gradient-to-b ${current.gradient}`}>
        ...
        <p className="text-[#8BA4B8] text-sm leading-relaxed">{current.desc}</p>
      </div>
        ...
        {STEPS.map((_, i) => (
          <div key={i}
            className={`h-1.5 rounded-full transition-all duration-300 ${
              i === step ? 'w-8 bg-[#0077B6]' : 'w-1.5 bg-white/20'
            }`}
          />
        ))}
      </div>
      ...
      <div className="flex gap-3">
        {step < STEPS.length - 1 ? (
          <>
            <Button variant="ghost" onClick={() => navigate('/login')} className="text-[#8BA4B8]">
              Saltar
            </Button>
          ...
          <Button size="lg" onClick={() => navigate('/register')}
            style={{ background: 'linear-gradient(135deg, #0077B6, #FFD100)' }}>
