...
export default function CreateEventWizard() {
  ...
  if (!user) return <p className="text-center text-muted py-16">Inicia sesión para crear eventos</p>
  if (!orgId && categories.length === 0) return <LoadingSpinner />
  if (!orgId) return <p className="text-center text-muted py-16">No tienes perfil de organizador. Solicítalo desde el dashboard.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
          ...
          <div key={s.step} className="flex items-center gap-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
              ...
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                ...
                step > s.step ? 'bg-green-500 text-white' : 'bg-primary/10 text-muted'
              }`}>
                {step > s.step ? '✓' : s.step}
              </span>
              {s.label}
            </div>
            {i < STEPS.length - 1 && <div className="w-8 h-px bg-primary/10" />}
          </div>
        ))}
      </div>
      ...
      {/* ===== STEP 1: INFO BÁSICA ===== */}
      {step === 1 && (
        <div className="bg-surface border border-primary/10 rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-text">Información del evento</h2>

          <div>
            <label className="block text-sm font-medium text-muted mb-1">Título del evento *</label>
            <input type="text" required value={form.title} onChange={e => update({ title: e.target.value })}
              ...
              className="w-full bg-base border border-primary/10 rounded-lg px-4 py-2.5 text-text text-sm placeholder:text-muted/50 focus:outline-none focus:border-primary transition-colors" />
          </div>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => update({ description: e.target.value })}
              ...
              className="w-full bg-base border border-primary/10 rounded-lg px-4 py-2.5 text-text text-sm placeholder:text-muted/50 focus:outline-none focus:border-primary transition-colors resize-none" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-muted mb-1">Categoría</label>
              <select value={form.category_id} onChange={e => update({ category_id: e.target.value })}
                className="w-full bg-base border border-primary/10 rounded-lg px-4 py-2.5 text-text text-sm focus:outline-none focus:border-primary">
            ...
            <div>
              ...
              <input type="text" value={form.tags} onChange={e => update({ tags: e.target.value })}
                ...
                className="w-full bg-base border border-primary/10 rounded-lg px-4 py-2.5 text-text text-sm placeholder:text-muted/50 focus:outline-none focus:border-primary" />
              <p className="text-[10px] text-muted mt-1">Separadas por comas</p>
            </div>
          </div>

            <div>
              <label className="block text-sm font-medium text-muted mb-1">URL de imagen de portada</label>
            ...
            <div>
              <label className="block text-sm font-medium text-muted mb-2">Artistas participantes</label>
              <div className="flex gap-2 mb-2">
                <input type="text" value={artistSearch} onChange={async e => {
                  ...
                  className="flex-1 bg-base border border-primary/10 rounded-lg px-4 py-2 text-text text-sm placeholder:text-muted/50 focus:outline-none focus:border-primary" />
              </div>
              {searchingArtist && <p className="text-xs text-muted mb-1">Buscando...</p>}
              {artistResults.length > 0 && (
                <div className="bg-base border border-primary/10 rounded-lg mb-2 overflow-hidden">
                  {artistResults.filter(a => !selectedArtists.find(s => s.id === a.id)).map(a => (
                    <button key={a.id} type="button" onClick={() => { setSelectedArtists([...selectedArtists, a]); setArtistSearch(''); setArtistResults([]) }}
                      className="w-full text-left px-4 py-2 text-sm text-text hover:bg-primary/5 transition-colors">
                      + {a.stage_name}
                    </button>
                  ))}
                </div>
              ...
              {selectedArtists.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {selectedArtists.map(a => (
                    <span key={a.id} className="inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full bg-primary/20 text-primary">
                      {a.stage_name}
      ...
      {step === 2 && (
        <div className="bg-surface border border-primary/10 rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold text-text">Fecha y ubicación</h2>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-muted mb-1">Fecha y hora de inicio *</label>
              <input type="datetime-local" required value={form.start_date} onChange={e => update({ start_date: e.target.value })}
                className="w-full bg-base border border-primary/10 rounded-lg px-4 py-2.5 text-text text-sm focus:outline-none focus:border-primary [color-scheme:dark]" />
            </div>
            <div>
              <label className="block text-sm font-medium text-muted mb-1">Fecha y hora de fin *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-muted mb-2">Selecciona la ubicación en el mapa *</label>
            {geo.position && (
              <LocationPicker center={geo.position} value={location} onChange={setLocation} />
            )}
            {!location && <p className="text-xs text-muted mt-1">Haz clic en el mapa para colocar el marcador</p>}
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="bg-surface border border-primary/10 rounded-xl p-6 space-y-4">
            <h2 className="text-lg font-bold text-text">Tipos de entrada</h2>

              {tiers.map((tier, i) => (
              <div key={tier._key} className="flex items-end gap-3 bg-base rounded-lg p-4 border border-white/5">
                <div className="flex-1">
                  <label className="block text-[11px] font-medium text-muted mb-1">Nombre</label>
                  <input type="text" value={tier.name} onChange={e => {
                    const next = [...tiers]; next[i] = { ...next[i], name: e.target.value }; setTiers(next)
                  }} placeholder="General"
                  className="w-full bg-surface border border-primary/10 rounded-lg px-3 py-2 text-text text-sm focus:outline-none focus:border-primary" />
                </div>
                <div className="w-24">
                  <label className="block text-[11px] font-medium text-muted mb-1">Precio (€)</label>
                ...
                <div className="w-24">
                  <label className="block text-[11px] font-medium text-muted mb-1">Cantidad</label>
                ...
                {tiers.length > 1 && (
                  <button type="button" onClick={() => setTiers(tiers.filter((_, j) => j !== i))}
                    className="text-secondary hover:text-text transition-colors p-2">
          ...
          <div className="bg-surface border border-primary/10 rounded-xl p-6 space-y-4">
            <h2 className="text-lg font-bold text-text">Publicar</h2>
              ...
              <button type="button" onClick={() => setPublishMode('draft')}
                className={`flex-1 rounded-lg border p-4 text-center transition-all ${
                  publishMode === 'draft'
                    ...
                    : 'border-primary/10 text-muted hover:border-white/20'
                }`}>
                <span className="text-2xl block mb-1">📝</span>
                <span className="text-sm font-medium">Guardar borrador</span>
                <p className="text-[11px] text-muted mt-1">Nadie lo verá hasta que lo publiques</p>
              </button>
              <button type="button" onClick={() => setPublishMode('published')}
                className={`flex-1 rounded-lg border p-4 text-center transition-all ${
                  publishMode === 'published'
                    ? 'border-green-500/50 bg-green-500/10 text-text'
                    : 'border-primary/10 text-muted hover:border-white/20'
                }`}>
                <span className="text-2xl block mb-1">🌍</span>
                <span className="text-sm font-medium">Publicar ahora</span>
                <p className="text-[11px] text-muted mt-1">Visible para todos al instante</p>
              </button>
            </div>
          </div>
        </div>
        ...
        <div>
          {step < 3 ? (
            ...
            <Button onClick={handleSubmit} loading={submitting} size="lg"
              style={{ background: publishMode === 'published' ? 'linear-gradient(135deg, var(--th-primary, #6366F1), var(--th-secondary, #EC4899))' : undefined }}>
